...
.controller('ui.bootstrap.persian.datepicker.DatepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$timeout', '$log', 'dateFilter', 'datepickerConfig','PersianDateService','persianDateFilter','$element', function($scope, $attrs, $parse, $interpolate, $timeout, $log, dateFilter, datepickerConfig,PersianDateService,persianDateFilter, $element) {
...
.directive( 'persianDatepicker', function () {
  return {
    ...
    link: function(scope, element, attrs, ctrls) {
        ...
        //console.log(scope.preselectedDates);
		...
		ngModelCtrl.$viewChangeListeners.push(function() {
			...
			if (scope.selectRange) {
				...
				if (!selectedDates.length || selectedDates.length > 1 || selectedDates[0] == dateVal){
					var result = selectedDates.splice(0, selectedDates.length, dateVal);
					scope.onChange({date: dateVal, action:'remove'}); 
					return result;
				}
				selectedDates.push(dateVal);
				scope.onChange({date: dateVal, action:'add'}); 
